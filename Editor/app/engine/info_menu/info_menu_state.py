    ...
    def create_personal_data_surf(self, growths=False):
        ...
        for idx, stat in enumerate(other_stats):
            ...
            if stat == 'TRV':
                ...
                render_text(surf, ['text'], [text_funcs.translate('WP')], ['yellow'], (72, 16 * true_idx + 24))
                desc = text_funcs.translate_and_text_evaluate('Willpower. Spent to use Skills. +1 on upkeep and +2 on kill.', unit=self.unit)
                self.info_graph.register((96 + 72, 16 * true_idx + 24, 64, 16), desc, state)
